...
The four robots exclusions established by the Crawl4AI baseline plus Quant
StackExchange's current `Disallow: /` remain in the cohort but are not sent to
Chromium. The result uses the same status, anti-bot marker, required-term and
minimum-content contract as the Crawl4AI probe. Full page content is discarded;
only a bounded preview, length and SHA-256 hash are retained.
